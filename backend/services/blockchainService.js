...
const contractAddressPath = path.join(__dirname, '../../shared/contract-addresses.json');
const projectRegistryArtifactPath = path.join(__dirname, '../../blockchain/artifacts/contracts/ProjectRegistry.sol/ProjectRegistry.json');
const carbonCreditTokenArtifactPath = path.join(__dirname, '../../blockchain/artifacts/contracts/CarbonCreditToken.sol/CarbonCreditToken.json');
const verificationOracleArtifactPath = path.join(__dirname, '../../blockchain/artifacts/contracts/VerificationOracle.sol/VerificationOracle.json');
...
class BlockchainService {
  ...
  async initializeContracts() {
    try {
      ...
      if (fs.existsSync(projectRegistryArtifactPath)) {
        const projectRegistryData = JSON.parse(fs.readFileSync(projectRegistryArtifactPath, 'utf8'));
        this.projectRegistry = new ethers.Contract(
          this.contractAddresses.contracts.ProjectRegistry,
          projectRegistryData.abi,
          this.signer || this.provider
        );
        console.log('📋 ProjectRegistry loaded:', this.contractAddresses.contracts.ProjectRegistry);
      }

      // Load CarbonCreditToken
      if (fs.existsSync(carbonCreditTokenArtifactPath)) {
        const carbonCreditTokenData = JSON.parse(fs.readFileSync(carbonCreditTokenArtifactPath, 'utf8'));
        this.carbonCreditToken = new ethers.Contract(
          this.contractAddresses.contracts.CarbonCreditToken,
          carbonCreditTokenData.abi,
          this.signer || this.provider
        );
        console.log('🪙 CarbonCreditToken loaded:', this.contractAddresses.contracts.CarbonCreditToken);
      }

      // Load VerificationOracle
      if (fs.existsSync(verificationOracleArtifactPath)) {
        const verificationOracleData = JSON.parse(fs.readFileSync(verificationOracleArtifactPath, 'utf8'));
        this.verificationOracle = new ethers.Contract(
          this.contractAddresses.contracts.VerificationOracle,
          verificationOracleData.abi,
          this.signer || this.provider
        );
        console.log('🔍 VerificationOracle loaded:', this.contractAddresses.contracts.VerificationOracle);
      }
  ...
  async registerProject(projectData) {
    ...
    try {
      ...
      // Map ecosystem type to enum value
      const ecosystemTypeMapping = {
        'mangrove': 0,
        'seagrass': 1, 
        'saltmarsh': 2
      };

      const ecosystemType = ecosystemTypeMapping[projectData.projectType] || 0; // Default to mangrove

      // Convert area to reasonable units (keep original hectares but scale appropriately)
      const areaInHectares = Math.floor(projectData.area * 100); // Scale by 100 instead of 10^18
      
      // Convert estimated credits to reasonable units  
      const estimatedCredits = Math.floor(projectData.estimatedCarbonCredits * 100); // Scale by 100 instead of 10^18

      // Create community wallets array (use project owner if no specific wallets)
      const communityWallets = [projectData.projectOwner];
      ...
      console.log('🔧 Contract parameters:', {
        ...
        ipfsMetadata: projectData.ipfsMetadata || '',
